...
function createSharedState () {
  ...
  function getNumericXref (item: any, key: string): number | undefined {
    ...
    const n = Number.parseFloat(String(xr.value).replace(/,/g, ''))
  ...
  const filteredItems = computed(() => {
    ...
    return items.filter(mod => {
      ...
      // must have SMILES
      if (hasSmilesOnly.value && !getSmiles(mod)) {
        return false
      }
      ...
      if (selectedOrigin.value.length > 0) {
        ...
        if (!selectedOrigin.value.includes(originVal)) {
          return false
        }
      }

      // TermSpec filter
      if (selectedTermSpec.value.length > 0) {
        const xr = findXref(mod, 'TermSpec')
        const termSpecVal = xr ? String(xr.value).trim() : ''
        if (!selectedTermSpec.value.includes(termSpecVal)) {
          return false
        }
      }
      ...
      if (applyDiff) {
        ...
        if (v < diffMonoRange.value[0] || v > diffMonoRange.value[1]) {
          return false
        }
      }
      ...
      if (applyMass) {
        ...
        if (v < massMonoRange.value[0] || v > massMonoRange.value[1]) {
          return false
        }
      }
